...
def _tree_generator(directory, prefix=""):
    """Helper function to generate tree lines recursively."""
    try:
        ...
        yield f"{prefix}[ไม่สามารถเข้าถึงได้]"
        return
    ...
    for i, entry in enumerate(entries):
        ...
        if os.path.isdir(os.path.join(directory, entry)):
            ...
            yield from _tree_generator(os.path.join(directory, entry), prefix + extension)

def run_task_list_contents(base_path, output_file, update_status):
    """
    Task 1 (MODIFIED): สร้างรายงานโครงสร้างไฟล์ (Tree) ของแต่ละโฟลเดอร์หลัก
    """
    update_status("Task 1: 🚀 กำลังสร้างรายงานโครงสร้าง Tree...")
    
    try:
        all_folders = sorted([d for d in os.listdir(base_path) if os.path.isdir(os.path.join(base_path, d))])
    except Exception as e:
        update_status(f"Task 1: ❌ ไม่สามารถอ่านโฟลเดอร์ได้: {e}")
        messagebox.showerror("Folder Error", f"Could not read directories in the target folder:\n{e}")
        return

    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"📊 รายงานโครงสร้างไฟล์และโฟลเดอร์ (Tree) ใน: {base_path}\n")
        f.write("="*80 + "\n\n")

        if not all_folders:
            f.write("ไม่พบโฟลเดอร์หลักในตำแหน่งที่เลือก\n")
            update_status("Task 1: ✅ ไม่พบโฟลเดอร์หลัก")
            return

        for i, folder_name in enumerate(all_folders):
            folder_path = os.path.join(base_path, folder_name)
            update_status(f"Task 1: 🌳 กำลังสร้าง Tree สำหรับ '{folder_name}' ({i+1}/{len(all_folders)})...")
            
            f.write(f"📁 โฟลเดอร์หลัก: {folder_name}\n")
            f.write(f".\n")
            
            for line in _tree_generator(folder_path):
                f.write(line + "\n")
            
            f.write("\n" + "="*80 + "\n\n")

    update_status(f"Task 1: 🎉 เขียนรายงาน Tree ลงในไฟล์เรียบร้อยแล้ว!")
...
def run_task_delete_empty_subfolders(base_path, output_file, update_status, scope, selected_subfolders):
    ...
    for i, folder in enumerate(all_folders):
        ...
        for sub in subfolders_to_check:
            ...
            if os.path.isdir(sub_path) and not os.listdir(sub_path):
                try:
                    absolute_sub_path = os.path.abspath(sub_path)
                    send2trash.send2trash(absolute_sub_path)
                    deleted_log.append(absolute_sub_path)
                except Exception as e:
...
def run_task_delete_empty_main_folders(base_path, output_file, update_status, selected_subfolders):
    ...
    for i, folder_name in enumerate(all_folders):
        ...
        for item in os.listdir(folder_path):
            ...
            if os.path.isfile(item_path):
                ...
                if item not in selected_subfolders and len(os.listdir(item_path)) > 0:
                    is_truly_empty = False
                    break
        ...
        if is_truly_empty:
            try:
                ...
                send2trash.send2trash(absolute_folder_path)
                deleted_log.append(absolute_folder_path)
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        f.write("🗑️ รายงานการย้ายโฟลเดอร์หลัก (ที่ไม่มีไฟล์ในโฟลเดอร์ย่อยที่เลือก และไม่มีไฟล์อื่น) ไปที่ถังขยะ:\n")
        f.write(f"(ตรวจสอบใน: {', '.join(selected_subfolders)})\n\n")
        if not deleted_log:
    ...
    for i, row in df.iterrows():
        ...
        if matched_folders:
            ...
            for sub_folder in matched_folders:
                ...
                # Use os.walk to reliably check for any file at any level
...
def run_task_create_folders_from_excel(base_path, output_file, update_status, excel_input_path, col1_name, col2_name, separator, subfolders_to_create):
    ...
    for i, row in df.iterrows():
        ...
        try:
            if not os.path.exists(folder_path):
                ...
                if subfolders_to_create:
                    for sub_name in subfolders_to_create:
                        sanitized_sub_name = sanitize_foldername(sub_name)
                        if sanitized_sub_name:
                            sub_path = os.path.join(folder_path, sanitized_sub_name)
                            try:
                                os.makedirs(sub_path)
                                log.append(f"  └─ CREATED Subfolder: {sanitized_sub_name}")
                            except Exception as sub_e:
                                log.append(f"  └─ ERROR Creating Subfolder '{sanitized_sub_name}': {sub_e}")
            else:
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        if subfolders_to_create:
            f.write(f"โฟลเดอร์ย่อยที่จะสร้าง: {', '.join(subfolders_to_create)}\n")
...
class App:
    def __init__(self, root):
        ...
        self.creation_subfolders = tk.StringVar(value="2D, 3D, Datasheet")
    ...
    def create_widgets(self):
        ...
        ttk.Label(path_frame, text="บันทึกไฟล์ Log/Output:").grid(row=1, column=0, sticky=tk.W, pady=2)
        ...
        subfolder_frame = ttk.LabelFrame(self.content_frame, text="2. เลือก Sub Folder (ถ้าจำเป็น)", padding="10")
    ...
    def _create_scrollable_frame(self, parent, max_height):
        ...
        checkbox_frame_id = canvas.create_window((0, 0), window=checkbox_frame, anchor="nw")
        ...
        canvas.bind("<Configure>", lambda e: canvas.itemconfig(checkbox_frame_id, width=e.width))
    ...
    def create_reporting_tab(self, parent_frame):
        ...
        tasks = [
            ("สร้าง Tree ของแต่ละ Folder", 1), 
            ("ค้นหา Sub Folder ที่ว่างเปล่า (ตามที่เลือก)", 2), 
            ("สรุปประเภทไฟล์ (ตามที่เลือก)", 3)
        ]
        for text, value in tasks:
            ttk.Radiobutton(task_frame, text=text, variable=self.selected_task, value=value, command=self.toggle_ui_elements).pack(anchor=tk.W)
        ...
        ttk.Radiobutton(self.scope_frame, text="สแกนทุก Folder", variable=self.search_scope, value="all", command=self.toggle_ui_elements).pack(side=tk.LEFT)
        ttk.Radiobutton(self.scope_frame, text="สแกนเฉพาะ Folder ที่เลือก", variable=self.search_scope, value="selected", command=self.toggle_ui_elements).pack(side=tk.LEFT, padx=10)
    ...
    def create_deletion_tab(self, parent_frame):
        ...
        ttk.Radiobutton(task_6_container, text="ย้าย Sub Folder ที่ว่างเปล่า", variable=self.selected_task, value=6, command=self.toggle_ui_elements).pack(side=tk.LEFT, anchor=tk.NW, pady=5)
        self.task_6_scope_frame = ttk.Frame(task_6_container)
        self.task_6_scope_frame.pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(self.task_6_scope_frame, text="เฉพาะ Sub Folder ที่เลือก", variable=self.delete_empty_scope, value="selected", command=self.toggle_ui_elements).pack(side=tk.LEFT)
        ttk.Radiobutton(self.task_6_scope_frame, text="ทุก Sub Folder ที่พบ", variable=self.delete_empty_scope, value="all", command=self.toggle_ui_elements).pack(side=tk.LEFT, padx=10)

        ttk.Radiobutton(task_frame, text="ย้าย Folder ที่ว่างเปล่า (ตามที่เลือก)", variable=self.selected_task, value=7, command=self.toggle_ui_elements).pack(anchor=tk.W, pady=5)
    ...
    def create_creation_tab(self, parent_frame):
        ...
        subfolder_creation_frame = ttk.LabelFrame(parent_frame, text="3. (ไม่บังคับ) สร้างโฟลเดอร์ย่อย", padding="10")
        subfolder_creation_frame.pack(fill=tk.X, pady=5)
        ttk.Label(subfolder_creation_frame, text="ใส่ชื่อโฟลเดอร์ย่อยที่ต้องการสร้าง (คั่นด้วย comma):").pack(anchor=tk.W)
        self.creation_subfolders_entry = ttk.Entry(subfolder_creation_frame, textvariable=self.creation_subfolders)
        self.creation_subfolders_entry.pack(fill=tk.X, expand=True, pady=(5, 0))
    ...
    def toggle_ui_elements(self):
        ...
        enable_subfolder_ui = (task_id in [2, 3, 5, 7, 8]) or \
    ...
    def start_task(self):
        ...
        needs_subfolders = (task_id in [2, 3, 5, 7, 8]) or \
                           (task_id == 4 and self.search_scope.get() == 'selected') or \
                           (task_id == 6 and self.delete_empty_scope.get() == 'selected')
        ...
        task_map = {
            1: run_task_list_contents, 2: run_task_find_empty_subfolders, 3: run_task_summarize_types, 
            4: run_task_find_specific_files, 5: run_task_delete_files_by_ext, 6: run_task_delete_empty_subfolders, 
            7: run_task_delete_empty_main_folders, 8: run_task_update_excel, 9: run_task_create_folders_from_excel
        }

        if task_id in [2, 3, 7]: args.append(selected_subfolders)
        elif task_id == 4:
            ...
            subfolders_to_create_str = self.creation_subfolders.get()
            subfolders_list = [name.strip() for name in subfolders_to_create_str.split(',') if name.strip()]
            args.extend([excel_input, col1, col2, separator, subfolders_list])
